...
object Converter {
  ...
  def fromObjectMeta(scriptIds: ScriptIds, metadata: ObjectMeta): SValue = {
    record(
      ...
      ("resourceVersion", SOptional(metadata.resourceVersionO.map(SText))),
  ...
  def toObjectMeta(v: SValue): Either[String, ObjectMeta] =
    v match {
      case SRecord(_, _, vals) if vals.size == 2 =>
        for {
          resourceVersion <- toOptional(vals.get(0), toText)
  ...
  def toUser(v: SValue): Either[String, User] =
    v match {
      case SRecord(_, _, vals) if vals.size == 4 =>
        ...
        } yield User(id, primaryParty, isDeactivated, metadata)
