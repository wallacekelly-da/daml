...
class ParticipantPartyStorageBackendTemplate extends ParticipantPartyStorageBackend {
  ...
  override def getPartyRecord(
      party: Ref.Party
  )(connection: Connection): Option[ParticipantPartyStorageBackend.DbPartyRecord] = {
    SQL"""
         SELECT
             internal_id,
             party,
      ...
      .map { case (internalId, party, resourceVersion, createdAt) =>
        ParticipantPartyStorageBackend.DbPartyRecord(
  ...
  override def createPartyRecord(
      partyRecord: ParticipantPartyStorageBackend.DbPartyRecordPayload
  ...
  override def compareAndIncreaseResourceVersion(internalId: Int, expectedResourceVersion: Long)(
    ...
    ParticipantMetadataBackend.compareAndIncraseResourceVersion("participant_party_records")(
      internalId,
