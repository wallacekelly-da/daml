...
private[platform] class BufferedTransactionsReader(
  ...
  private def getTransactions[FILTER, API_TX, API_RESPONSE](
    ...
    def filterBuffered(slice: Vector[(Offset, TransactionLogUpdate)]) =
      slice.iterator
  ...
  private def toFlatTransaction(
    ...
    events.headOption.flatMap { first =>
      if (first.commandId.nonEmpty || events.nonEmpty) {
        val flatEvents =
          events.map(toFlatEvent(_, filter.keySet, verbose)).collect { case Some(ev) =>
            ev
          }
...
private[events] trait DelegateTransactionsReader extends LedgerDaoTransactionsReader {
