...
//        }
      ...
      withValidation {
        ...
        val submit = for {
          ...
          partyRecord <- participantPartyRecordStore
            .createPartyRecord(
      ...
      withValidation {
        ...
        val partyRecordUpdate = UpdateMapper.toParticipantPartyUpdate(
          partyRecord = ParticipantParty.PartyRecord(
            party = party,
            metadata = domain.ObjectMeta(
              resourceVersionO = resourceVersionO,
              annotations = annotations,
            ),
          ),
        ...
        for {
          updatedPartyRecordResult <- participantPartyRecordStore.updatePartyRecord(
  ...
  private def handleResult[T](operation: String)(
      result: v2.ParticipantPartyRecordStore.Result[T]
  )(implicit errorLogger: DamlContextualizedErrorLogger): Future[T] =
    result match {
      // TODO pbatko: Party vs. PartyRecord
      case Left(ParticipantPartyRecordStore.PartyNotFound(party)) =>
        Future.failed(
      ...
      case Left(ParticipantPartyRecordStore.PartyRecordNotFound(party)) =>
        Future.failed(
          LedgerApiErrors.Admin.PartyManagement.ParticipantPartyNotFound
            .Reject(operation, party = party)
            .asGrpcError
      ...
      case Left(ParticipantPartyRecordStore.PartyRecordExists(party)) =>
        Future.failed(
      ...
      case Left(ParticipantPartyRecordStore.ConcurrentPartyUpdate(party)) =>
        Future.failed(
      ...
      case Left(ParticipantPartyRecordStore.ExplicitMergeUpdateWithDefaultValue(party)) =>
