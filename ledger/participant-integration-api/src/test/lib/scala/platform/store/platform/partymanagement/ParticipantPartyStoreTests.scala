...
trait ParticipantPartyStoreTests extends TestResourceContext with Matchers with EitherValues {
  ...
  "participant party management" - {
    ...
    "updating" - {
      ...
      "should metadata annotations with merge and replace semantics" in {
        testIt { tested =>
      ...
      "should raise error when updating a non-existing party record for which a ledger party doesn't exist" in {
        testIt { tested =>
